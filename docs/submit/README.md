...
|---|---|
| `images/` | 인게임 화면 8장 |
| `fonts.css` · `fonts/` | 두 문서가 나눠 쓰는 웹폰트 하위 집합 |
...
`IBM Plex Mono` 400·500·600)를 받아, **두 HTML이 쓰는 글자를 합친 집합**과 `unicode-range`가
겹치는 `@font-face`만 남기고 그 woff2를 내려받는다. 91개 파일 1.3 MB가 나온다.
한쪽 문서에 새 글자가 들어가면 이 선별을 다시 돌려야 한다.

## 두 문서의 색 규칙이 다르다

같은 종이와 같은 글꼴을 쓰지만 색이 뜻하는 것이 다르다.
