 ...
 *
 * 지금 HR-03이 다섯 스테이지의 시작 구간 전부에서 걸리기 때문이다(step.ts의 armed 주석).
...
function hr03Section(suite: BalanceSuite): string[] {
  ...
  return [
    ...
    '',
    '  셋째 줄은 관측이 아니라 추정이다. 가드의 면제 집계는 sim/step.ts의 모듈 사설 WeakMap 안이라 밖에서 못 읽는다.',
    '  첫 탄이 나오기 전의 공백은 세지 않는다 — 그 시작점은 sim/step.ts의 armed와 같은 자리다.',
  ];
}
