...
/**
 * 밸런스 리포트가 도는 시드. **단일 실행으로 판단하지 않는다** — 한 판의 패리 시행이 수십
 * 단위라 성공률도 명중률도 시드 하나에서는 잡음과 구분되지 않는다. 다섯이면 표준편차가
 * 리포트에 같이 실리고, 밴드 이탈이 편차 안인지 밖인지를 사람이 아니라 표가 말한다.
 *
 * 값 자체에 뜻은 없다. 뜻이 있는 것은 **이 목록이 커밋되어 있다**는 사실이다.
 */
export const BALANCE_SEEDS: readonly number[] = [20260807, 4713, 918273, 60451, 1229];
