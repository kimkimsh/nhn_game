 ...
 *
 * shotsPerCycle과 fireCycleSec은 §8.1 표의 값이 아니다 — 체감으로 다시 세웠고 주석의 「원값」이
 * §8.1이 적어 둔 숫자다(difficulty.ts 헤더). hp · moveSpeedUPerSec · spreadDeg · score는 §8.1 그대로다.
 *
 * **첫 발 시각은 이 표에 없다.** 분대가 한 몸처럼 쏘지 않도록 `sim/enemies.ts`가 분대 안의 자리로
 * 흩는다 — 그 이유와 근거는 그쪽 `firstShotSec` 주석에 있다.
 */
...
export const ENEMIES = {
  'E-A': {
    ...
    fireCycleSec: 3.08,           // 발사 주기 (s). §8.1 원값 2.2
    isRanged: true,
    contactDamage: false,
    score: 100,                   // §8.1 처치 점수
  },
  'E-B': {
    ...
    bullet: 'P1',
    shotsPerCycle: 2,             // 발수. §8.1 원값 3
    spreadDeg: 14,                // §8.1 ±14° 부채꼴 — 반각 (deg). 2발이면 최외곽 두 발이 곧 전부다
    fireCycleSec: 3.64,           // 발사 주기 (s). §8.1 원값 2.6
    isRanged: true,
    contactDamage: false,
    score: 100,                   // §8.1 처치 점수
  },
  ...
  'E-E': {
    ...
    shotsPerCycle: 1,
    spreadDeg: 0,                 // 단발이라 부채꼴이 없다
    fireCycleSec: 4.76,           // 발사 주기 (s). §8.1 원값 3.4
    isRanged: true,
    contactDamage: false,
    score: 200,                   // §8.1 처치 점수
  },
  'E-F': {
    ...
    bullet: 'P11',
    shotsPerCycle: 1,             // 통과 중 발수. §8.1 원값 2
  ...
  'E-G': {
    ...
    behavior: 'holdAndFire',
    bullet: 'P4',
    shotsPerCycle: 7,             // 링 탄막 발수. §8.1 원값 10
    spreadDeg: 360,               // §8.1 전방위 링. 유일하게 반각이 아니라 전방위 표시다. 7발이면 등간격 51.4°
    fireCycleSec: 4.2,            // 발사 주기 (s). §8.1 원값 3.0
    isRanged: true,
    contactDamage: false,
    score: 180,                   // §8.1 처치 점수
  },
  'E-H': {
    ...
    behavior: 'holdAndFire',
    bullet: 'P8',
    shotsPerCycle: 1,             // 발수. §8.1 원값 2. 1발이면 부채꼴이 사라지고 조준 단발이 된다
    spreadDeg: 10,                // §8.1 ±10° 부채꼴 — 반각 (deg). 목업의 ±5°는 A6이 기각했다
    fireCycleSec: 3.92,           // 발사 주기 (s). §8.1 원값 2.8
    isRanged: true,
    contactDamage: false,
    score: 160,                   // §8.1 처치 점수
  },
  'E-I': {
    ...
    behavior: 'mounted',
    bullet: 'P6',
    shotsPerCycle: 3,             // 일제 발수. §8.1 원값 4
    spreadDeg: 0,                 // 일제 사격이고 부채꼴로 벌리지 않는다
    fireCycleSec: 5.6,            // 발사 주기 (s). §8.1 원값 4.0
    isRanged: true,
    contactDamage: false,
    score: 300,                   // §8.1 처치 점수
