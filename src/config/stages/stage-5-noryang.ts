...
export const STAGE_5 = {
  ...
  waves: [
    ...
    {
      // §9.7 W4 — 관통 대창이 처음 들어온다. 편성 13기로 한 번 성글게 만든 뒤 광역 공격을 얹는다
      //
      // **E-I가 §9.7의 3기가 아니라 5기인 것은 W5가 상한을 넘겨야 하기 때문이다.** E-I는
      // 모함 부착이라 스스로 나가지 않는 유일한 잔류형이고(§8.1 moveSpeedUPerSec 0), W5로
      // 넘어가는 잔존이 곧 이 수다. 3기면 잔존 3 + W5 편성 16 = 19라 상한 20에 닿지 못하고,
      // 그러면 §9.7이 「신규 스폰이 지연되는 상황이 의도적으로 발생하는 유일한 구간」이라고
      // 부른 그 구간이 편성상 존재할 수 없다 — §3.2가 잡몹 상한을 20으로 올린 이유가 그
      // 장면이므로, 못 만들면 상한 자체가 근거를 잃는다. 조치를 이 두 값이 아니라 여기서
      // 하라고 적어 둔 자리가 dev/report.ts의 S5_MIN_SPAWN_DEFERRALS 주석이다.
      startSec: 42,
      endSec: 56,
      ...
      concurrentMax: 13, // §9.7 동시 최대. 원값 11
